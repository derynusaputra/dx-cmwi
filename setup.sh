...
setup_wsl() {
    ...
    case "$choice" in
        ...
        2)
            ...
            SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
            WIN_PATH="$(wslpath -w "$SCRIPT_DIR/setup-windows-server.ps1")"
...
setup_windows_from_bash() {
    ...
    SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    WIN_SCRIPT="$SCRIPT_DIR/setup-windows-server.ps1"
    [[ -f "$WIN_SCRIPT" ]] || die "setup-windows-server.ps1 tidak ditemukan"
