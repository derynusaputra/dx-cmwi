...
export default function UserDropdown() {
  ...
  const handleLogout = async () => {
    ...
    // Hapus semua cookie auth agar middleware tidak redirect balik ke dashboard
    const cookieOptions = 'path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT; SameSite=Lax';
    document.cookie = `refresh_token=; ${cookieOptions}`;
    document.cookie = `active_role=; ${cookieOptions}`;
    document.cookie = `intended_role=; ${cookieOptions}`;
    logout();
    router.push('/signin');
  };
      ...
      >
        <div>
          ...
          <span className="mt-0.5 block text-theme-xs text-gray-500 dark:text-gray-400">
            {user?.roles && user.roles.length > 0 ? user.roles.join(', ') : 'No Role'}
          </span>
        </div>
